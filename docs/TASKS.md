...
## G7 — 검증·민감도 🟡 부분 (E5·SNR·화자·하위군 완료, 전문가 검토만 남음)

| 항목 | 상태 | 근거 |
|---|---|---|
| 모든 시드를 실행하는 사전등록 설정 | ✅ | `experiments/configs/rq1_main.yaml` |
| 절제·자막·임계값 연구 기록 | ✅ | `experiments/registry.yaml` |
| 교정 길이 연구 (10/25/50/100/전체) | ✅ | E25 에서 10/25/50/100/200/400 전부 완료; 이득이 교정량에 거의 불변 (RESULTS §13 B1) |
| SNR 스윕 | ✅ | E25 에서 +20/+10/+5/0/−5 완료; 전체 이득이 조건 내부 이득의 6–14배 (§13 B2) |
| 화자 민감도 | ✅ | E25; 화자 간 차이 작고 방향 일관 (§13 B3) |
| 중증도·WRS 대역별 하위군 분석 | ✅ | E25; **하위군 붕괴 확인** — severe +0.0004(2/5), acc_low +0.0000(3/5) (§13 B4) |
...
| 정확 목표음소 특징 절제 (Phase D) | ✅ | E28; **음성 결과** — 블록을 더할수록 Recall@20% 단조 감소 (RESULTS §16) |
| 교차적합 잔차 · 쌍별 랭커 (Phase E/F) | ✅ | E29; 어느 후보도 §22 교체 기준 미달, **로지스틱 유지** (§17) |
| 앙상블 상보성 (Phase G) | ✅ | 잔차 상관 0.90–1.00, 순증 회복 ±9단어 → **앙상블 중단** (§18) |
| 시뮬레이터 V2 (생성 과정 풍부화) | ✅ | E27; 구조는 생겼으나 회복률 불변(80.8%→80.5%), 기본값은 v1 유지 (RESULTS §15) |

...
| 개인 특이성 스윕(디리클레 집중도) | 🟡 | E11 구현·테스트 완료(`make sensitivity`). 전체 격자 실행 대기 |
...
1. **개인 특이성 × 교정 길이 스윕 전체 격자 실행.** 코드와 설정은 준비되었습니다
   (`make sensitivity`, 3 × 3 격자 × 시드 3개 = 27칸). 실행하면 개인화 선택 자막이
   비개인화 휴리스틱을 이기는 영역이 존재하는지, 그 경계가 어디인지 답이 나옵니다.
   **남은 실험 중 정보량이 가장 큽니다.**

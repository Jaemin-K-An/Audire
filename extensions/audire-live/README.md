...
## 현재 단계 (Phase 8 — 페이지 배선)

**실제 사이트의 자막은 아직 읽지 않습니다.** 실사이트 어댑터가 없고, 사이트 호스트 권한도
없습니다. 콘텐츠 스크립트는 저장소 안의 픽스처 페이지에만 주입됩니다.
...
**꺼져 있으면 어댑터조차 고르지 않습니다.** 서비스 워커도 꺼짐을 막지만 그 지점에서는
자막이 이미 확장 안으로 들어온 뒤입니다. 읽지 않는 것과 읽고 나서 버리는 것은 다릅니다.
...
src/
  ...
  observer/pageController.js    선택·관찰·메시지를 잇습니다. 꺼짐이면 아무것도 읽지 않습니다.
  content-loader.js    콘텐츠 스크립트 진입점(평범한 스크립트).
  content.js           브라우저 물건을 꽂는 얇은 배선.
...
| `pageController.test.js` | 꺼져 있는데 페이지를 읽는 것. 큐 id 에 자막이 실리는 것. 뿌리 재획득이 무한 반복되는 것. 콘텐츠 스크립트가 `chrome.storage` 를 만지는 것 |
| `source-hygiene.test.js` | 보이지 않는 제어문자가 원본에 들어오는 것 (실제로 한 번 일어났습니다) |

이 시험들은 **실제로 깨지는지** 확인했습니다. Phase 6 에서 6개, Phase 7 에서 8개, Phase 8
에서 10개의 변형을 주입해 각각 해당 시험이 실패하는 것을 보았습니다.
...
| `content_scripts.matches` | `http://{localhost,127.0.0.1}/*/ott-page.html` — 저장소 안의 픽스처뿐 |
| `web_accessible_resources` | 확장 자신의 `src/*.js`, 루프백에만 |

실제 사이트 권한은 **아직 없습니다**. 어댑터가 생기는 단계에서 그 사이트만
`content_scripts.matches` 에 들어갑니다. `<all_urls>` 와 실제 사이트 이름은 시험이 막습니다.

매니페스트 선언 자체(주입·`web_accessible_resources`·동적 import)는 압축해제된 확장을
Chrome 에 적재해야 확인됩니다 — Phase 13 의 수동 스모크 몫입니다.
